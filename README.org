...
~guess-language-mode-line-string~ sets the string that will be displayed if the dictionary being used is the default one (determined by Flyspell). The default value for this variable is ~"default"~.
